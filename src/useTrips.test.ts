...
describe('useTrips', () => {
  ...
  it('setArchived patches only the targeted trip', async () => {
    const { result } = renderHook(() => useTrips());
    await waitFor(() => expect(result.current.loaded).toBe(true));
    const firstId = result.current.trips[0].id;
    ...
    expect(result.current.trips.find((t) => t.id === secondId)?.archived).toBe(true);
    expect(result.current.trips.find((t) => t.id === firstId)?.archived).toBeUndefined();
  });

  it('archiving the active trip reassigns activeTripId to a remaining non-archived trip', async () => {
    const { result } = renderHook(() => useTrips());
    await waitFor(() => expect(result.current.loaded).toBe(true));
    const firstId = result.current.trips[0].id;
  ...
  it('archiving the last non-archived trip leaves activeTripId unchanged', async () => {
    const { result } = renderHook(() => useTrips());
    await waitFor(() => expect(result.current.loaded).toBe(true));
    const id = result.current.trips[0].id;
    ...
    expect(result.current.trips[0].archived).toBe(true);
  });

  it('unarchiving clears the flag', async () => {
    const { result } = renderHook(() => useTrips());
    await waitFor(() => expect(result.current.loaded).toBe(true));
    const id = result.current.trips[0].id;

    act(() => result.current.setArchived(id, true));
    act(() => result.current.setArchived(id, false));

    expect(result.current.trips[0].archived).toBe(false);
  });

  it('deleteTrip still refuses at exactly one trip regardless of archived status', async () => {
    const { result } = renderHook(() => useTrips());
    await waitFor(() => expect(result.current.loaded).toBe(true));
    const id = result.current.trips[0].id;
